...
struct Detalles {
	...
	int cantAlumns=0, cantAlumAprob=0, cantAlumDesap=0;
	float promNotaAlumn;
...
void imprTodaInfo(){
	...
	printf("\n % DE ALUMNOS APROBADOS: %.1f%", details.porcAlumAprob);
	printf("\n % DE ALUMNOS DESAPROBADOS: %.1f%", details.porcAlumDesap);
}
...
int main() {
	...
	float sumaNota=0;
	details.alumnNotaAlta.nota = 0;	/* asignando valor de partida */
	details.alumnNotaBaja.nota = 10.1;	/* para compararlo después */
